...
Since the value of R square coefficient is relatively low (`r summary(fit)$r.squared`) it is quite likely that there are other variables, which can help in explaining the variation in MPG.

Additional variables to the model can be determined by looking at the absolute value of correlation of numeric variables with *mpg* variable.
...
### Model wtih including automobile's weight

Following code fits the linear model using *am* and *wt* variables.
...
### Model wtih including engine displacement

The following code fits the model using *am* and *disp* variables. *disp* variable measures the engine displacement.
...
Also in the case of this model, the p value of `r summary(fit)$coef[2,4]` for coefficient *`r dimnames(summary(fit3)$coef)[[1]][2]`* indicates that it has no significant effect on exaplaining variation in MPG. Therefore, despite relatively large $R^2$ value of `r summary(fit3)$r.square`, this model is also not appropriate for explaining the impact of transmission type on automobile's MPG.

```{r fit_model_chart, fig.height = 3.5, fig.width = 3.5}
...
The most important criterion for model selection is that it must allow us to see the impact of transmission type on MPG. Therefore, such model should include *am* variable.
